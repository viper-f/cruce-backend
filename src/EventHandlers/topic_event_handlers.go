...
func refreshSubforumStats(db *sql.DB, subforumID int) {
	var topicCount int
	_ = db.QueryRow("SELECT COUNT(*) FROM topics WHERE subforum_id = ? AND status != ?", subforumID, Entities.DeletedTopic).Scan(&topicCount)

	var postCount int
	_ = db.QueryRow("SELECT COUNT(*) FROM posts p JOIN topics t ON p.topic_id = t.id WHERE t.subforum_id = ? AND COALESCE(p.is_deleted, 0) != 1 AND t.status != ?", subforumID, Entities.DeletedTopic).Scan(&postCount)
	...
	err := db.QueryRow(`
		...
		WHERE t.subforum_id = ? AND COALESCE(p.is_deleted, 0) != 1 AND t.status != ?
		ORDER BY p.date_created DESC
		LIMIT 1
	`, subforumID, Entities.DeletedTopic).Scan(&lastPostID, &lastPostTopicID, &lastPostTopicName, &lastPostDate, &lastPostAuthor)

	if err == sql.ErrNoRows {
		var lastTopicID *int64
		var lastTopicName *string
		err2 := db.QueryRow("SELECT id, name FROM topics WHERE subforum_id = ? AND status != ? ORDER BY date_created DESC LIMIT 1", subforumID, Entities.DeletedTopic).Scan(&lastTopicID, &lastTopicName)
...
func RegisterTopicEventHandlers() {
	...
	// Subscriber 3b: Refresh subforum stats when topics are deleted
